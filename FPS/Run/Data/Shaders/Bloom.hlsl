...
fragment_output_t FragmentFunction( v2f_t input )
{
	...
	float3 color = diffuse * surface_color + specular_color;
	color = pow(color.xyz, INVERSE_GAMMA);
	float4 final_color = float4( color, surface_alpha );

	float3 overflow = max( specular_color.xyz - float3( 1.f, 1.f, 1.f ), float3( 0.f, 0.f, 0.f ) );
	float4 bloom = float4( overflow, 1.f );
