...
Texture2D <float4> tInput   : register(t0);   // color of the surface
Texture2D <float4> tToAdd   : register(t8);   // color of the surface
...
{
   float4 color = tInput.Sample(sSampler, input.uv);
   float4 colorToAdd = tToAdd.Sample(sSampler, input.uv);

   float4 final_color = color + colorToAdd;
