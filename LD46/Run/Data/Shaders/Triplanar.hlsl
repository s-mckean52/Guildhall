...
float4 FragmentFunction( v2f_t input ) : SV_Target0
{
	...
	float4 x_color = tXDiffuse.Sample( sSampler, x_uv );
	float4 y_color = tYDiffuse.Sample( sSampler, y_uv );
	float4 z_color = tZDiffuse.Sample( sSampler, z_uv );
	...
	float3 surface_color = pow( triplanar_color.xyz, GAMMA.xxx );
	surface_color *= input.color.xyz;
	float surface_alpha = ( input.color.a * triplanar_color.a );

	float3 world_normal = triplanar_normal;
